<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ url('/') }}">
                        <x-application-logo
                            class="block h-9 w-auto fill-current text-violet-900 dark:text-violet-400 group" />
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    <x-nav-link :href="Auth::user()->role == 'admin' ? url('/admin/dashboard') : url('/dashboard')">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                @else
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        ...
        {{-- <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="mtt-3 space-y-1">
                @auth


                <x-responsive-nav-link :href="Auth::user()->role == 'admin' ? url('/admin/dashboard') : url('/dashboard')" :active="request()->routeIs('home')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
